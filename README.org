...
**** Parallel and asynchronous upgrades

When upgrading multiple packages, Quelpa can analyze package dependencies and upgrade independent packages in parallel, making the upgrade process faster and non-blocking.

To enable parallel upgrades, set both =quelpa-parallel-upgrade-p= and =quelpa-async-p= to =t=:

#+BEGIN_SRC elisp
  (setq quelpa-async-p t)              ; Enable async operations (non-blocking)
  (setq quelpa-parallel-upgrade-p t)   ; Enable parallel upgrades
  (setq quelpa-max-parallel-upgrades 4) ; Limit concurrent upgrades
#+END_SRC

When parallel upgrades are enabled:
- Quelpa builds a dependency graph of packages to be upgraded
- Packages with no dependencies are upgraded first, in parallel
- Packages are upgraded as soon as their dependencies are satisfied
- The upgrade process does not block Emacs input
- Progress messages show the current status

Note: Parallel upgrades are only used when upgrading multiple packages (e.g., via =quelpa-upgrade-all=).
